...
* `webpack`配置项`mode`：`string: none|developement|production`，用于指定`webpack`构建情景并自动使用相应的优化
  * `developement`: 主要体现在增量构建和开发体验（例如热模块替换、source map、proxy等）上的优化
  * `production`：主要体现在代码压缩、Tree Shaking等方面的优化
